...
import { RecoveryTwinSimulator, SyntheticPaymentRecord, SyntheticDatasetGenerator } from '@recoveryos/simulator';
...
export const simulatorRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  fastify.post('/simulator/run', async (request, reply) => {
    ...
    const records: SyntheticPaymentRecord[] = body.customRecords || SyntheticDatasetGenerator.generate(1000, 1337);
